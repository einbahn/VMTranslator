...
class Parser(object):
    def __init__(self, inputfile):
        cwd = os.getcwd()
        self.file = open(pathlib.Path(cwd, inputfile))
        self.currentCommand = ""
    ...
    def advance(self):
        try: 
            while True:
                self.currentCommand = re.sub(r"\s+.*", "", self.file.readline().lstrip())
    ...
    def commandtype(self):
        ...
        if a == "push":
            ...
            return "C_POP"
        elif a in ("add", "sub", "neg", "eq", "gt", "lt", "and", "or", "not"):
            return "C_ARITHMETIC"
...
class CodeWriter(object):
    def __init__(self, outputfile):
        cwd = os.getcwd()
        self.file = open(pathlib.Path(cwd, outputfile), "w")
        self.labelcounter = 0
        self.filename = outputfile.replace('.asm','')
    ...
    def writepushpop(self, command, segment, index):
        ...
        if command == 'push':
            if segment == 'constant':
                ...
                M=M+1
                """
                self.file.writelines(trim(asm).format(commandstring, segmentName, index, self.labelcounter))
                self.labelcounter += 1
            elif segment == 'static':
...
if __name__ == "__main__":
    ...
    outputfullname = os.path.join("tests", outputfile)
    parser = Parser(inputf)
    cw = CodeWriter(outputfullname)
    while parser.hasMoreCommands:
        parser.advance()
        cw.writeArithmetic(parser.currentCommand)
    cw.close()
    parser.close()
